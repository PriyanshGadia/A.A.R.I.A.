...
# [UPGRADED] - This module is now the dedicated "Executor" of the agent.
# All "Brain" (decision-making) logic has been removed and centralized
# in CognitionCore. This file now only executes actions given to it.
...
# --- [NEW] Imports for upgraded LLM access ---
try:
    from llm_adapter import LLMAdapterFactory, LLMRequest, LLMProvider
except Exception:
...
# Prometheus metrics (remains the same)
...
# Async retry decorator (remains the same)
...
# Circuit breaker (remains the same)
    ...
    async def can_execute(self) -> bool:
        async with self._lock:
            ...
            if self.state == "OPEN":
                ...
                if self.last_failure_time is None: return False
    ...
    async def get_state_info(self) -> Dict[str, Any]:
        async with self._lock:
            return { "state": self.state, "failure_count": self.failure_count,
                     "last_failure_time": self.last_failure_time, "last_state_check": self._last_state_check }
...
# AES-GCM encryptor (remains the same)
class AESGCMEncryptor:
    ...
    def decrypt(self, encrypted_data: Dict[str, Any]) -> str:
        try:
            ...
            if et == "plaintext_fallback": return encrypted_data["data"]
            if et == "base64_fallback": return base64.b64decode(encrypted_data["data"]).decode("utf-8")
...
# Async multi-level priority heap-backed queue (remains the same)
...
# Action dataclass (remains the same)
@dataclass
class Action:
    action_type: str
    ...
    def __post_init__(self):
        if self.timestamp is None: self.timestamp = time.time()
        ...
        if self.category == ActionCategory.SYSTEM.value:
            if self.action_type.startswith("notify"): self.category = ActionCategory.NOTIFICATION.value
            elif self.action_type.startswith("calendar"): self.category = ActionCategory.CALENDAR.value
            elif self.action_type.startswith("contact"): self.category = ActionCategory.CONTACT.value
            elif "llm" in self.action_type or self.source == "llm_autonomy": self.category = ActionCategory.LLM.value
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "id": self.id, "action_type": self.action_type, "details": self.details,
            "priority": self.priority, "source": self.source, "timestamp": self.timestamp,
            "executed": self.executed, "result": self.result, "category": self.category,
...
class AutonomyCore:
    ...
    async def initialize(self):
        ...
        self.action_requirements = {
            "notify": {"required": ["channel", "message"], "optional": ["urgency", "subject_id"], "category": ActionCategory.NOTIFICATION.value},
        ...
        try:
            if hasattr(self.core, "config_manager"):
                ...
                async with self._config_lock:
                    ...
                    if llm_cfg: self.llm_config.update(llm_cfg)
                    rate_limits = config.get("rate_limits", {})
                    if rate_limits: self.rate_limits_config.update(rate_limits)
    ...
    async def enqueue_action(self, action: Action) -> bool:
        ...
        logger.info("Enqueued action %s (id=%s)", action.action_type, action.id) # Changed to info
        return True
    ...
    async def _validate_action(self, action: Action) -> Tuple[bool, str]:
        ...
        if action.action_type == "notify":
            # --- [UPGRADED] ---
            # Added "push" and "logging" as valid channels for the proactive communicator
            valid_channels = ["desktop", "sms", "call", "push", "logging"]
            # --- [END UPGRADE] ---
    ...
    def _get_service_from_action(self, action_type: str) -> str:
        if action_type.startswith("calendar"): return "calendar"
        if action_type.startswith("notify"): return "notifications"
        if action_type.startswith("contact"): return "contacts"
        return "default"
    ...
    # --- [UPGRADED] ---
    # `execute_action` is upgraded to use the safe hologram wrappers.
    # --- [END UPGRADED] ---
    async def execute_action(self, action: Action) -> Tuple[bool, Any]:
        ...
        spawned = False # Track if spawn succeeded

        async with self._execution_context(action):
            start = time.monotonic()
            try:
                # --- [UPGRADED] Safe hologram spawn ---
                spawned = await self._safe_holo_spawn(
                    node_id=node_id, node_type="task", label=action.action_type,
                    size=6, source_id="AutonomyCore", link_id=link_id
                )
                if spawned:
                    await self._safe_holo_set_active("AutonomyCore")
                # --- [END UPGRADED] ---
                ...
                # --- [UPGRADED] Safe hologram error ---
                if spawned:
                    await self._safe_holo_set_error(node_id)
                # --- [END UPGRADED] ---
                return False, err
            finally:
                # --- [UPGRADED] Safe hologram cleanup ---
                if spawned:
                    await self._safe_holo_set_idle("AutonomyCore")
    ...
    async def _process_action_result(self, action: Action, result: Any, exec_time: float, success: bool) -> Dict[str, Any]:
        return {
            "success": bool(success), "execution_time": exec_time, "timestamp": time.time(),
            "action_type": action.action_type, "action_id": action.id, "category": action.category,
            "details": result if success else {"error": result}, "priority": action.priority
        }
    ...
    async def _get_user_context(self) -> Dict[str, Any]:
        try:
            if hasattr(self.persona, "get_user_context"):
                ...
                if asyncio.iscoroutine(ctx): return await ctx
                return ctx
        except Exception:
    ...
    async def _generate_compliance_tags(self, action: Action) -> Dict[str, str]:
        return {
            ...
            "retention_period": "7y", "encryption_required": "true",
    ...
    # --- [UPGRADED] ---
    # This method now correctly uses LLMAdapterFactory
    # --- [END UPGRADED] ---
    async def _execute_notify(self, details: Dict[str, Any]) -> Dict[str, Any]:
        message = details.get("message", "")
        # Summarize long messages via LLM if available
        if len(message) > 200: # Removed check for llm_orchestrator
            summary = await self._summarize_message_with_fallback(message)
            if summary:
                message = summary
        
        # --- [NEW] Proactive Notification ---
        # Instead of just logging, we now enqueue with the *actual* ProactiveCommunicator
        # We find the communicator via the injected PersonaCore
        status = "sent_simulated (no proactive module)"
        if self.persona and hasattr(self.persona, "proactive") and self.persona.proactive:
            try:
                await self.persona.proactive.enqueue_message(
                    subject_id=details.get("subject_id", "owner_primary"),
                    channel=details.get("channel", "push"),
                    payload={"text": message, "urgency": details.get("urgency", "normal")},
                    deliver_after=time.time() # Deliver now
                )
                status = "enqueued_proactively"
                logger.info(f"Enqueued notification for {details.get('channel')}")
            except Exception as e:
                logger.warning(f"Failed to enqueue proactive notification: {e}")
    ...
    async def _summarize_message_with_fallback(self, message: str) -> Optional[str]:
        # This method already correctly uses LLMAdapterFactory.
        if LLMAdapterFactory is None or LLMRequest is None:
            logger.warning("LLMAdapterFactory not available for summarization.")
            return None
        ...
        for model in models_to_try:
            if not model:
                continue
            try:
                # --- [FIXED] Determine provider based on model name ---
                if "claude" in model:
                    provider = LLMProvider.ANTHROPIC
                elif "gpt" in model:
                    provider = LLMProvider.OPENAI
                elif "llama" in model:
                    provider = LLMProvider.GROQ # Default to Groq for Llama
                else:
                    provider = LLMProvider.OLLAMA # Fallback for local models
                
                async with LLMAdapterFactory.get_adapter(provider, model=model) as adapter:
    ...
    # --- [UPGRADED] ---
    # These methods now correctly call the sync methods
    # on self.core (AssistantCore) as they were designed to do.
    # --- [END UPGRADED] ---
    async def _execute_calendar_add(self, details: Dict[str, Any]) -> Dict[str, Any]:
        try:
            if hasattr(self.core, "add_event"):
                # add_event in assistant_core is sync, so we await the executor
                loop = asyncio.get_event_loop()
                event_id = await loop.run_in_executor(None, self.core.add_event, details)
                return {"status": "added", "event_id": event_id}
            return {"status": "simulated", "event_id": f"event_{int(time.time())}", "note": "Calendar integration not available"}
        except Exception as e:
    ...
    async def _execute_calendar_list(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if hasattr(self.core, "list_upcoming_events"):
                # list_upcoming_events in assistant_core is async
                res = await self.core.list_upcoming_events(days)
                return {"status": "listed", "count": len(res) if isinstance(res, list) else 0, "days": days, "events": res}
    ...
    async def _execute_contact_get(self, details: Dict[str, Any]) -> Dict[str, Any]:
        try:
            if hasattr(self.core, "list_contacts"):
                # list_contacts in assistant_core is async
                res = await self.core.list_contacts()
    ...
    async def _execute_contact_search(self, details: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            if hasattr(self.core, "list_contacts"):
                # list_contacts in assistant_core is async
                res = await self.core.list_contacts()
                filtered = [c for c in res if query in c.get("name", "").lower() or query in (c.get("email", "") or "").lower()]
                return {"status": "searched", "count": len(filtered[:limit]), "results": filtered[:limit]}
            return {"status": "simulated", "count": 0, "results": []}
        except Exception as e:
            return {"status": "failed", "error": str(e)}

    # Autonomy cycle & continuous loop (remains the same)
    async def run_autonomy_cycle(self) -> Dict[str, Any]:
        ...
        summary = {
            "status": "completed", "executed": executed, "successful": successful,
            "cycle_time": cycle_time, "queue_remaining": await self.pending_actions(),
            "currently_executing": self.currently_executing, "timestamp": time.time()
        }
    ...
    # ---------------------------------------------------------------
    # --- [DELETED] "BRAIN" METHODS ---
    # The following methods are DELETED. This logic is now centralized
    # in CognitionCore, which is the single "Brain".
    #
    # - evaluate_autonomous_decision(...)
    # - _build_autonomy_prompt(...)
    # - _parse_llm_action_robust(...)
    #
    # ---------------------------------------------------------------

    # Monitoring & health (remains the same)
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        health_status = "healthy" if total == 0 or success / total > 0.9 else "degraded" # Adjusted logic
        return {
            "status": health_status, "queue_size": await self.pending_actions(),
            "recent_executions": total, "success_rate": (success / max(1, total)),
            "rate_limits_active": len(self.rate_limits), "autonomy_enabled": self.autonomy_enabled,
            "total_executed": len(self.executed_actions), "failed_actions": len(self.failed_actions),
            "failure_rate": len(self.failed_actions) / max(1, len(self.executed_actions) + len(self.failed_actions)),
            "circuit_breakers": cb_states, "currently_executing": self.currently_executing,
            "timestamp": now
        }

    # Lifecycle methods (remains the same)
    ...
    async def _perform_shutdown_audit(self) -> int:
        ...
        async with self._exec_lock:
            ...
            snapshot = {
                "shutdown_ts": ts, "final_queue_size": len(queue_snapshot),
                "currently_executing": self.currently_executing, "total_executed": len(self.executed_actions),
                "total_failed": len(self.failed_actions), "sample_count": len(recent_actions),
    ...
    async def reset_circuit_breakers(self):
        ...
        if changed: logger.info("Reset %s circuit breakers", changed)
        else: logger.debug("No circuit breakers required reset")

    async def cleanup(self):
        self.autonomy_enabled = False
        for h in list(self._config_handles):
            try:
                if hasattr(h, "cancel"): h.cancel()
            except Exception:
