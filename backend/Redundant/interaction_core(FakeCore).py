...
if __name__ == "__main__":
    # Quick internal test harness (uses minimal fake cores)
    ...
    class FakeStorage:
        ...
        def put(self, key, typ, val):
            self._store[(key, typ)] = val
        def get(self, key, typ):
            return self._store.get((key, typ))
        def delete(self, key, typ):
            self._store.pop((key, typ), None)

    class FakePersonaCore:
        def __init__(self):
            self.core = self
            self.storage = FakeStorage()
            self.system_prompt = "You are A.A.R.I.A. (fake persona)"
        def load_user_profile(self):
            return {"name": "TestUser", "timezone": "UTC", "is_admin": True, "user_id": "user_123"}
        def notify(self, channel, message):
            logger.info(f"[FAKE NOTIFY] channel={channel} message={message[:120]}")

    class FakeCognition:
        def __init__(self):
            self._cache = {}
        def reason(self, query, context=None, use_llm=False, max_retries=0):
            return json.dumps({"summary": "simulated plan", "steps": ["step1", "step2"]})
        def _get_rate_limit_fallback(self, query):
            return "[Fake cognition rate limit fallback]"
        def _get_fallback_reasoning(self, query):
            return "[Fake cognition fallback reasoning]"

    class FakeAutonomy:
        def __init__(self):
            pass

    persona = FakePersonaCore()
    cognition = FakeCognition()
    autonomy = FakeAutonomy()
    config = {"async_mode": False, "hmac_audit_key": None}
    interaction = InteractionCore(persona=persona, cognition=cognition, autonomy=autonomy, config=config)

    # Simulate inbound messages
    inbound = InboundMessage(channel="text", content="Hello there!", user_id="user_123")
    out = interaction.handle_inbound(inbound)
    print("Outbound:", out.content)

    inbound2 = InboundMessage(channel="text", content="Can you schedule a meeting for tomorrow?", user_id="user_123")
    out2 = interaction.handle_inbound(inbound2)
    print("Outbound:", out2.content)

    # Test privileged command (should be allowed for fake admin user)
    inbound3 = InboundMessage(channel="text", content="run autonomy.execute", user_id="user_123")
    out3 = interaction.handle_inbound(inbound3)
    print("Outbound:", out3.content)

    # Test rate limiting
    for i in range(5):
        inbound_test = InboundMessage(channel="text", content=f"Test message {i}", user_id="rate_test_user")
        out_test = interaction.handle_inbound(inbound_test)
        print(f"Rate test {i}: {out_test.content}")
